...
for i, d in enumerate(data):
    ...
    lang_dict = {}
    for lang, url in d['wikilinks'].items():
        lang = lang[:-4]
        name = url.rsplit('/', 1)[-1]
        try:
            wiki_wiki = wikipediaapi.Wikipedia(lang)
            lang_page = wiki_wiki.page(name, unquote=True)
            if lang_page.exists():
                lang_dict[lang] = lang_page.summary
        except Exception as e:
            print(e)
            print(f'---->Failed to get {lang} page for id {id}')
    data_with_summaries[id] = lang_dict
    count += 1
